...
async function deleteOffer(req, res) {
    try {
        ...
        const deleteOffer = await db.query(`DELETE FROM public.offres WHERE id = ${id}`);
...
async function updateOffer(req, res) {
    try {
        ...
        const {id, titre, description, entreprise, lieu, date_limite, type_contrat, salaire, experience, diplome_requis, competences } = req.body;

        const updateOffer = await db.query(`UPDATE public.offres SET titre=$1, description=$2, entreprise=$3, lieu=$4, date_limite=$5, type_contrat=$6, salaire=$7, experience=$8, diplome_requis=$9, competences=$10, updated_at=NOW() WHERE id=${id}`, 
        [
            titre, 
            description, 
            entreprise, 
